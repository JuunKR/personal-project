...
class Preprocessing(object):
    ...
    def check_ksx1001(self):
        ...
        temp_train = []
        temp_val = []
        for index, word in enumerate(label_data[:,3]):
            if word in delete_words:
                temp_train.append(label_data[index,:])

        for index, word in enumerate(label_validation[:,3]):
            if word in delete_words:
                temp_val.append(label_validation[index,:])
        
        ksx1001_train = np.array(temp_train)
        ksx1001_val = np.array(temp_val)
                
        if label_data.shape == ksx1001_train.shape:
            print('train data는 ksx1001 글자만 사용했습니다.')
        else:
            print('train data는 ksx1001 외의 글자가 포함되어 있습니다.')

        if label_validation.shape == ksx1001_val.shape:
            print('validation data는 ksx1001 글자만 사용했습니다.')
        else:
            print('validation data는 ksx1001 외의 글자가 포함되어 있습니다.')

    # def to_cvs(self):

    def separate_train_test(path, data):
        words = pd.read_csv('csv/ksx100.csv', encoding='UTF-8')
    ...
    def generate_train_data(self, kernel):
        ...
        np.save('../data_csv/x_train_data.npy', arr=xy_train[0][0])
        np.save('../data_csv/y_train_data.npy', arr=xy_train[0][1])

        return xy_train


    def generate_test_data(self, kernel):
        ...
        np.save('../data_csv/x_test_data.npy', arr=xy_test[0][0])
        np.save('../data_csv/y_test_data.npy', arr=xy_test[0][1])

        return xy_test

    def generate_val_data(self, kernel):
        val_path = '../data/Validation/images/words/data'

        val_datagen = ImageDataGenerator(rescale=1./255)

        xy_val = val_datagen.flow_from_directory(
            val_path,
            batch_size=33114,
            target_size=(kernel,kernel),
            class_mode='categorical'
        ...
        np.save('../data_csv/x_val_data.npy', arr=xy_val[0][0])
        np.save('../data_csv/y_val_data.npy', arr=xy_val[0][1])

        return  xy_val

    def train_augmentation(self, x_train, y_train, kernel):
        train_datagen = ImageDataGenerator(rescale=1./255,
                                 rotation_range = 20,
                                 width_shift_range=0.1,
                                 height_shift_range=0.1,
        ...
        x_argmented = x_argmented.reshape(x_argmented.shape[0], kernel, kernel, 3)
        x_train = x_train.reshape(x_train.shape[0], kernel, kernel, 3) 
        ...
        np.save('../data_csv/x_a_train_data.npy', arr=x_train)
        np.save('../data_csv/y_a_train_data.npy', arr=y_train)               
...
if __name__ == '__main__':
    # file_paths = ['../data/Training/labels/words', '../data/Validation/labels/words']
    p = Preprocessing()
    # p.make_ksx1001()